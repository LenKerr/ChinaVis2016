...
restList = list(set(allNodeList).difference(set(Server)))
...
def isServer(node):
    result = cur.execute("select count(*) as c,sum(filelen) as f,count(dstip) as d from `rpcServer` where srcip ='"+node+\
            "' having c >400 and f > 2595616*2 and d > 6")
    if result <= 0:
        return False
    else:
        rows = cur.fetchall()
        filelen = rows[0][1]
        result = cur.execute("select sum(filelen) as flow from rawdataall where dstip like '"+node+"' group by DSTPORT having flow > 0.2*"+str(filelen))
        if result <= 0:
            return False
    return True

def isNotNode(node):
    result = cur.execute("select count(dstip) as d from `rpcServer` where srcip = '"+node+"' having d>6")
    if result >0:
        return False
    return True
...
while len(restList) > len(allNodeList)*(1-complete):
    ...
    if randomNode in Server:
        ...
        if neighborNode(randomNode,Server): 
            ...
            if isServer(randomNode):
                Server.append(randomNode)
                unkServer.append(randomNode)
                restList.remove(randomNode)
                continue
            else:
                if isNotNode(randomNode):
                    restList.remove(randomNode)
                result = cur.execute("select dstip from `rpcServer` where srcip in "+str(tuple(Server))+\
                " and dstip not in "+str(tuple(Server))+" group by dstip order by count(distinct srcip)*\
                  sum(filelen) desc limit 1")
                rows = cur.fetchall()
                toAdd = rows[0][0]
                #print toAdd
                Server.append(toAdd)
                if isServer(toAdd):
                    unkServer.append(toAdd)
                else:
                    referServer.append(toAdd)
